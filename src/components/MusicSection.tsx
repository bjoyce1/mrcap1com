...
const MusicSection = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(featuredRef.current, { scale: 0.9, opacity: 0 }, {
      ...
      if (albumCards) {
        gsap.fromTo(albumCards, { y: 60, opacity: 0, scale: 0.95 }, {
          y: 0, opacity: 1, scale: 1, duration: 0.8, stagger: 0.1, ease: "power3.out",
          scrollTrigger: { trigger: albumsRef.current, start: "top 80%", toggleActions: "play none none reverse" },
        });
      }

      const singleYears = singlesRef.current?.querySelectorAll(".singles-year");
      if (singleYears) {
        gsap.fromTo(singleYears, { x: -40, opacity: 0 }, {
          x: 0, opacity: 1, duration: 0.8, stagger: 0.1, ease: "power3.out",
          scrollTrigger: { trigger: singlesRef.current, start: "top 80%", toggleActions: "play none none reverse" },
        });
      }
    }, sectionRef);
  ...
  return <section ref={sectionRef} id="music" className="section-spacing relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div ref={featuredRef} className="mb-20 will-change-transform">
        ...
        <div ref={albumsRef} className="mb-20">
          <div className="flex items-center gap-3 mb-8">
            <Disc3 className="w-5 h-5 text-primary" />
            <h3 className="font-editorial text-2xl">Studio & Collab Albums</h3>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {albums.map((album, index) => <div key={index} className="album-card group glass rounded-xl overflow-hidden will-change-transform card-lift">
              <div className="relative aspect-square overflow-hidden bg-muted">
                {album.image ? <img src={album.image} alt={`${album.title} Album Cover`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" /> : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary/10 to-primary/5">
          ...
          <div className="space-y-8">
            {singles.map((yearGroup, yearIndex) => <div key={yearIndex} className="singles-year will-change-transform">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {yearGroup.tracks.map((track, trackIndex) => <div key={trackIndex} className="group flex items-center gap-3 p-4 rounded-xl glass cursor-pointer hover:border-primary/20 transition-all">
